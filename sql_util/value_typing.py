...
def type_values_w_db(orig_path: str, typed_values: List[Tuple[Tuple[str, str], str]], loose: bool) \
    ...
    for (lhs_table, lhs_col), value in typed_values:
        #for (table, col), elements in table_col2column_elements.items():
        #    if table == lhs_table
        ...
        for t_c in all_possible_ancestors:
            if value_is_num or value in table_col2column_elements[t_c]:
                if value not in new_values[t_c]:
                    new_values[t_c].append(value)

                    if value_is_num:
                        try:
                            int_num = int(value)
                            new_values[t_c].append(str(int_num + 1))
                            if int_num > 0:
                                new_values[t_c].append(str(int_num - 1))
                        except:
                            try:
                                float_num = float(value)
                                new_values[t_c].append(str(float_num + 1))
                                if float_num > 0:
                                    new_values[t_c].append(str(float_num - 1))
                            except:
                                pass
