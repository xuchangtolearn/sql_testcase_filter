...
from utils.process_sql import tokenize, get_schema, get_tables_with_alias, Schema, get_sql
...
def evaluate(gold, predict, db_dir, etype, kmaps):
    ...
    for p, g in zip(plist, glist):
        ...
        try:
            ...
            eval_err_num += 1
            print("eval_err_num:{}".format(eval_err_num), p_str)
...
def get_execution_result(sql_str, db_id=None, db_path=None, format='col_order'):
    assert format in ['col_order', 'row_order']
    assert format == 'row_order'
    assert db_id is not None or db_path is not None
    # test suite
    sql_str = sql_str.replace('> =', '>=').replace('< =', '<=').replace('! =', '!=')

    if db_path is not None:
        db = db_path
    else:
        db = os.path.join('data/database', db_id, db_id + ".sqlite")
    ...
    def res_map(res, val_units):
        ...
        for idx, val_unit in enumerate(val_units):
            ...
            rmap[key] = [r[idx] for r in res]
        return rmap
    if format == 'row_order': return res
    ...
    val_units = [unit[1] for unit in sql['select'][1]]

    return res_map(res, val_units)
