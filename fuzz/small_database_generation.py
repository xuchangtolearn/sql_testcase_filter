...
def generate_db_to_distinguish_queries(sql, neighbor_queries, orig_db_path, new_db_root, data_index, data_type, db_type, test_case_num,
                                       table_size=5, MAX_NUM=20):
    all_db = []
    try_num = 0
    dir_path = new_db_root
    ...
    while len(all_db) < test_case_num and try_num < MAX_NUM:
        ...
        # generate_path = os.path.join(dir_path, test_case_dir, '{}_{}.db'.format(os.path.basename(new_db_root), data_index))
        generate_path = os.path.join(dir_path, test_case_dir, '{}.db'.format(os.path.basename(new_db_root)))
        try_num += 1
        # if db_type in ['fuzzing', 'select_values']:
